...
} from "../StoreManagers/MessagesManager";
...
function ChatBox(props) {
  const {
    ...
    startChatting,
    handleTest,
    startOrStopChattingButtonEnabled,
  } = props;
  ...
  const allMessages = messages.map((message) => {
    return (
      <div
        key={message.id}
        style={!message.otherUser ? { justifyContent: "flex-end" } : null}
        className="text-messages-container"
      >
        <div style={{ width: "100%", display: "block" }}>
          <div
            className="message-data"
            style={!message.otherUser ? { textAlign: "right" } : null}
          >
            <span className="message-data-time">{message.time}</span>
            <span className="message-data-name">
              {message.otherUser ? ` - ${message.name}` : null}
            </span>
          </div>
          <div
            className="message"
            style={!message.otherUser ? { float: "right" } : null}
          >
            {message.content}
          </div>
        </div>
  ...
  return (
    <div>
      <div className="chat-list">
        ...
        <div className="location-messages">
          {" "}
          - {user.name} (You){" "}
          {myLocation !== "N/A" && <>- Location: {myLocation}</>}
        </div>
        {otherUsersInfo.name !== "N/A" ? (
          <Fragment>
            <div className="location-messages">
              {" "}
              - {otherUsersInfo.name} (Joined){" "}
              {otherUsersInfo.location !== "N/A" && (
                <>- Location: {otherUsersInfo.location}</>
              )}{" "}
            </div>
      ...
      <form onSubmit={submitMessage} style={{ width: "785px", height: "3em" }}>
        ...
        {chatting ? (
          <button
            className="start-stop-chatbox-button"
            disabled={!startOrStopChattingButtonEnabled}
